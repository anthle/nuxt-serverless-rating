...
import { getAvg } from "@/utlis";

const { data, pending, refresh } = useFetch('/api/ranks', {
  default() {
    return {};
  },
});
const formattedData = computed<RankItem[]>(() => {
  ...
  for (const key in data.value) {
    const value = (data.value as any)[key] || '';
    if (value) {
...
<template>
  <div class="container p-4 sm:px-0 sm:mx-auto sm:pb-0">
    <header class="flex items-center mb-2">
      ...
      <span class="rounded-full w-3 h-3 animate-ping bg-sky-400 ml-4" v-if="pending"></span>
    </header>
